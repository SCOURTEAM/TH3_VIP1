sudo_TH3_VIP1 = dofile("sudo.lua")
https = require("ssl.https")
JSON = dofile("./libs/JSON.lua")
local TH3_VIP1_dev = io.open("tshake_online.lua")
if TH3_VIP1_dev then
  
tshake_on = {string.match(TH3_VIP1_dev:read('*all'), "^(.*)/(%d+)")}
local tsheke_file = io.open("sudo.lua", 'w')
tsheke_file:write("token = '" ..TH3_VIP1_on[1].."'\n\nsudo_add = "..tshake_on[2].."" )
tsheke_file:close()
https.request("https://api.telegram.org/bot"..tshake_on[1].."/sendMessage?chat_id="..tshake_on[2].."&text=Bot_Tshake_is_start_new")
os.execute('cd .. && rm -rf .telegram-cli')
os.execute('rm -rf TH3_VIP1_online.lua')  
os.execute('./tg -s ./TH3_VIP1.lua $@ --bot='..tshake_on[1])
end
...
os.execute('cd && rm -fr .telegram-cli')
os.execute('./tg -s ./TH3_VIP1.lua $@ --bot='..tokenCk)
else
...
local data = JSON:decode(req)
if data.ok == true then
os.execute('./tg -s ./TH3_VIP1.lua $@ --bot='..token)
else
